...
new_index_num_found=`curl -s -k -H "X-Authentication-Token: $auth_token" \
    --digest --user $auth \
   "https://harvest-stg.cdlib.org/solr/dc-collection/select?q=run&wt=json" | \
   jq '.response.numFound'`
...
new_index_num_found=`curl -s -k -H "X-Authentication-Token: $auth_token" \
    --digest --user $auth \
   "https://harvest-stg.cdlib.org/solr/dc-collection/select?q=runs&wt=json" | \
   jq '.response.numFound'`
...
new_index_num_found=`curl -s -k -H "X-Authentication-Token: $auth_token" \
    --digest --user $auth \
   "https://harvest-stg.cdlib.org/solr/dc-collection/select?q=running&wt=json" | \
   jq '.response.numFound'`
