#"https://harvest-stg.cdlib.org/solr/dc-collection/select?q=d%C3%ADa&wt=json"
...
new_index_num_found=`curl -s -k -H "X-Authentication-Token: $auth_token" \
    --digest --user $auth \
   "https://harvest-stg.cdlib.org/solr/dc-collection/select?q=dia&wt=json" | \
   jq '.response.numFound'`
...
new_index_num_found=`curl -s -k -H "X-Authentication-Token: $auth_token" \
    --digest --user $auth \
   "https://harvest-stg.cdlib.org/solr/dc-collection/select?q=el+d%C3%ADa+de+los+muertos&rows=0&wt=json" | \
   jq '.response.numFound'`
echo "NEW Index select \"el día de los muertos\" found $new_index_num_found"
new_index_num_found=`curl -s -k -H "X-Authentication-Token: $auth_token" \
    --digest --user $auth \
   "https://harvest-stg.cdlib.org/solr/dc-collection/select?q=el+dia+de+los+muertos&rows=0&wt=json" | \
   jq '.response.numFound'`
...
new_index_num_found=`curl -s -k -H "X-Authentication-Token: $auth_token" \
    --digest --user $auth \
   "https://harvest-stg.cdlib.org/solr/dc-collection/select?q=El+D%C3%ADa+de+los+Muertos+1983&rows=0&wt=json" | \
   jq '.response.numFound'`
echo "NEW Index select \"El Día de los Muertos 1983\" found $new_index_num_found"
new_index_num_found=`curl -s -k -H "X-Authentication-Token: $auth_token" \
    --digest --user $auth \
   "https://harvest-stg.cdlib.org/solr/dc-collection/select?q=El+Dia+de+los+Muertos+1983&rows=0&wt=json" | \
   jq '.response.numFound'`
